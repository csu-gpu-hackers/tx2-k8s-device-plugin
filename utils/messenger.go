...
type Messenger struct {
	...
	connection net.Conn
	Actions map[string]func(string, string, string)string
}
...
func InitMessenger(socketPath string, actions map[string]func(string, string, string)string) *Messenger {
...
func (messenger *Messenger) Serve()  {
	...
	for true {
		...
		conn.Write([]byte(messenger.Actions[message.MessageType](message.PodUID, message.ContainerID, message.MessageContent)))
	}
